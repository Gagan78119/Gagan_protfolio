...
const SkillCard = ({ skill, delay, isInView }: SkillCardProps) => {
  ...
  // Determine progress bar color based on skill level
  const getProgressColor = (level: number) => {
    if (level >= 90) return 'from-blue-500 to-purple-500';
    if (level >= 80) return 'from-blue-500 to-blue-400';
    if (level >= 70) return 'from-blue-400 to-blue-300';
    return 'from-blue-300 to-blue-200';
  };

  // Create skill icon based on skill name
  ...
  return (
    <div 
      className="bg-white p-5 rounded-lg shadow-md border border-gray-100 relative overflow-hidden group hover:shadow-lg transition-all"
    ...
    >
      ...
      <div className="absolute -right-10 -bottom-10 w-32 h-32 rounded-full bg-blue-500/5 opacity-60 group-hover:bg-blue-500/10 transition-all" />
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs py-1 px-2 bg-gray-100 rounded-full text-gray-600 border border-gray-200">
            {skill.category}
          </span>
        </div>
        ...
        <div className="h-2 bg-gray-100 rounded-full mb-3 overflow-hidden">
          <div 
            className={`h-full rounded-full transition-all duration-1000 ease-out bg-gradient-to-r ${getProgressColor(skill.level)}`}
            style={{ 
